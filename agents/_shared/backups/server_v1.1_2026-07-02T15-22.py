...
def vault_read_data(rel: str) -> dict:
    ...
    pol = _vault_edit_policy(rel, "read")
    return {
        "ok": True,
        "path": rel,
        "text": full.read_text(encoding="utf-8", errors="replace"),
        "editable": pol.get("editable", False),
        "edit_mode": pol.get("mode"),
    }
...
VAULT_CANDIDATE_PREFIX = "generated-projections/"
_FM = re.compile(r"^---\s*\n(.*?)\n---\s*\n", re.DOTALL)


def _vault_norm_rel(rel: str) -> str:
    return (rel or "").replace("\\", "/").lstrip("/")


def _vault_parse_fm(text: str) -> tuple[dict[str, str], str]:
    m = _FM.match(text)
    if not m:
        return {}, text
    meta: dict[str, str] = {}
...
def _vault_body_len(text: str) -> int:
    _, body = _vault_parse_fm(text)
    return len(re.sub(r"\s+", "", body))


def _vault_edit_policy(rel: str, action: str = "save") -> dict:
    """Overwatch MC edit permissions (Clio Wave E)."""
    rel = _vault_norm_rel(rel)
    if not rel or ".." in rel.split("/"):
        return {"ok": False, "editable": False, "mode": "deny", "error": "invalid path"}

    low = rel.lower()
    if low in ("index.md", "log.md") or low.endswith("/index.md"):
        return {"ok": True, "editable": False, "mode": "readonly", "error": "read-only meta page"}
    if low.startswith("foundation/"):
        return {"ok": True, "editable": False, "mode": "deny", "error": "foundation is read-only"}

    if rel.startswith(VAULT_CANDIDATE_PREFIX) and not rel.endswith("/INDEX.md"):
        if action == "delete":
            return {"ok": True, "editable": True, "mode": "full", "allow_delete": True}
        return {"ok": True, "editable": True, "mode": "full", "allow_delete": True}

    if any(rel.startswith(p) for p in VAULT_WRITE_PREFIXES):
        return {"ok": True, "editable": True, "mode": "append", "allow_delete": False}

    return {"ok": True, "editable": False, "mode": "deny", "error": "zone not editable via MC"}


def _vault_run_schema_lint(rel: str, old_text: str | None, new_text: str) -> dict:
    script = ROOT / "scripts" / "vault_schema_lint.py"
    cmd = [sys.executable, str(script), "--vault", str(VAULT_ROOT), "--path", rel, "--json"]
    proc = subprocess.run(cmd, capture_output=True, text=True, cwd=str(ROOT))
    try:
        return json.loads(proc.stdout or "{}")
    except json.JSONDecodeError:
        return {"ok": False, "hard": [{"code": "LINT_ERROR", "message": proc.stderr or proc.stdout}], "soft": []}


def _vault_clio_harmonize_task(rel: str, soft: list) -> None:
    slug = Path(rel).stem
    day = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    note_path = f"agents/clio/content/{day}_vault-lint-{slug}.md"
    codes = ", ".join({s.get("code", "?") for s in soft[:8]})
    board_create(
        {
            "title": f"Clio: harmonize vault lint — {slug}",
            "notes": f"vault:{rel} · soft: {codes} · artifact: {note_path}",
            "priority": "medium",
            "status": "pending",
            "source_key": f"vault-lint:{rel}",
        }
    )


def _vault_append_log(line: str) -> None:
    log = VAULT_ROOT / "log.md"
    if log.is_file():
        with log.open("a", encoding="utf-8") as f:
            f.write(line if line.startswith("\n") else f"\n{line}")


def vault_save_data(rel: str, body: bytes) -> dict:
    rel = _vault_norm_rel(rel)
    pol = _vault_edit_policy(rel, "save")
    if not pol.get("editable"):
        raise ValueError(pol.get("error") or "not editable")

    new_text = body.decode("utf-8")
    full = _vault_safe_path(rel)
    old_text = full.read_text(encoding="utf-8", errors="replace") if full else ""

    if pol.get("mode") == "append" and old_text:
        if _vault_body_len(new_text) < int(_vault_body_len(old_text) * 0.7):
            raise ValueError("trusted zone is append-only — large body deletions blocked")

    new_meta, _ = _vault_parse_fm(new_text)
    if (new_meta.get("trust") or "").lower() == "trusted" and old_text:
        old_meta, _ = _vault_parse_fm(old_text)
        if (old_meta.get("trust") or "").lower() != "trusted":
            raise ValueError("trust: trusted only via promote_vault_candidate.py")

    lint = _vault_run_schema_lint(rel, old_text or None, new_text)
    if lint.get("hard"):
        codes = ", ".join(h.get("code", "?") for h in lint["hard"])
        raise ValueError(f"schema lint hard fail: {codes}")
    ...
    _vault_append_log(f"\n## [{stamp}] mc-save | {rel} | overwatch\n")

    soft = lint.get("soft") or []
    if soft:
        _vault_clio_harmonize_task(rel, soft)

    return {"ok": True, "path": rel, "soft_lint": len(soft)}


def vault_candidate_create(payload: dict) -> dict:
    ...
    day = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    now = utc_now_iso()
    filename = f"{day}_{slug}-candidate.md"
    rel = f"{VAULT_CANDIDATE_PREFIX}{filename}"
    dest = VAULT_ROOT / rel
    if dest.exists():
        raise ValueError("candidate file already exists")

    text = f"""---
name: "{title}"
type: synthesis
id: candidate-{slug}-{day}
trust: candidate
added: "{now}"
last_updated: {day}
citations: ""
...
"""
    ...
    _vault_append_log(f"\n## [{day}] mc-candidate | {rel} | overwatch\n")
    return {"ok": True, "path": rel, "text": text}


def vault_delete_data(rel: str) -> dict:
    rel = _vault_norm_rel(rel)
    pol = _vault_edit_policy(rel, "delete")
    if not pol.get("allow_delete"):
        raise ValueError("delete not allowed for this path")
    full = _vault_safe_path(rel)
    if not full:
        raise ValueError("not found")
    full.unlink()
    stamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    _vault_append_log(f"\n## [{stamp}] mc-delete | {rel} | overwatch\n")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/vault/read":
            ...
            self._json(200 if out.get("ok") else 404, out)
            return
        if path == "/api/vault/validate":
            qs = parse_qs(urlparse(self.path).query)
            rel = (qs.get("path") or [""])[0]
            body = (qs.get("text") or [""])[0]
            if not rel:
                self._json(400, {"ok": False, "error": "path required"})
                return
            full = _vault_safe_path(rel)
            old = full.read_text(encoding="utf-8", errors="replace") if full else ""
            lint = _vault_run_schema_lint(rel, old or None, body or old)
            self._json(200, {"ok": not lint.get("hard"), **lint})
            return
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/api/vault/save":
                rel = (qs.get("path") or [""])[0]
                self._json(200, vault_save_data(rel, self._read_body()))
                return
            if path == "/api/vault/candidate":
                self._json(201, vault_candidate_create(self._read_json()))
                return
            if path == "/api/vault/delete":
                rel = (qs.get("path") or [""])[0]
                self._json(200, vault_delete_data(rel))
                return
        except ValueError as exc:
            self._json(400, {"ok": False, "error": str(exc)})
            return
