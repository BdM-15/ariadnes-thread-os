...
AGENT_META = {
    "hermes": {"code": "HER", "display": "Hermes", "role": "Guildmaster / Orchestrator"},
    "iris": {"code": "IRIS", "display": "Iris", "role": "Scout / Intel"},
    "clio": {"code": "CLIO", "display": "Clio", "role": "Scribe / Packet"},
    "odysseus": {"code": "ODY", "display": "Odysseus", "role": "Knight / Strategist"},
    "hephaestus": {"code": "HEP", "display": "Hephaestus", "role": "Artificer / Builder"},
}
        ...
        for agent_dir in sorted(agents_root.iterdir()):
            ...
            for path in content.rglob("*"):
                ...
                if path.suffix.lower() == ".md":
                    hm = re.match(r"^#\s+(.+)$", text, re.M)
                    if hm:
                        title = hm.group(1).strip()[:120]
                st = path.stat()
                docs.append(
                    {
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/content":
            ...
            self._json(200, read_content_file(rel))
            return
        if path == "/events":
