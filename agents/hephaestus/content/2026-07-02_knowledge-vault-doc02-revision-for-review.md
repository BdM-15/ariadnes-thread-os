...
**Wave 1 inspiration doc 02 · DRAFT for Overwatch review (revision)**  
**Not merged** to `docs/inspiration/` until you approve this revision.
...
**Implementation note (post–doc approve):** Hermes skill `knowledge-triage` or `ROUTER.md` lane—Hephaestus wires after Overwatch merges inspiration doc.
...
## Overwatch review (revision)

Decisions baked in from your direction:

- [x] North star = compounding brain, not folder cosplay  
- [x] Hermes triage **B** (default)  
- [x] Root = `knowledge/thread/`  
- [x] Defer `data-elements/` until Wave 2  
- [x] Wandermist zone **INDEX.md** scaffolding (§4.1)  
- [x] Spectral graph = **Overwatch demo / delight** only, v2+ (§4.2)—not agent production path  

**Reply `approve`** to merge to `docs/inspiration/knowledge-vault-and-compounding-truth.md`, update `00-INSPIRATION-INDEX.md`, commit on `docs/capform-inspiration`.  
**Reply with edits** if anything should change before merge.